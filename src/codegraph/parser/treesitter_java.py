...
class JavaParser(BaseParser):
    """Parse Java sources for types, methods, and calls."""
    ...
    def parse_text(
        ...
        rel = rel_path(Path(rel_path_)) if "/" not in rel_path_ else rel_path_.replace("\\", "/")
        self._calls.clear()
    ...
    def _walk(self, node, raw, rel, out, parent_uid, current_fn, commit_sha, valid_from):
        ...
        if node.type == "method_invocation":
            fn = node.child_by_field_name("name") or node.child_by_field_name("object")
